...
def add_route_info(reaction_names: List[str],
    ...
    if retro_tool is not None:
        route['retro_tool'] = retro_tool
    if db_search_tool is not None:
        route['db_search_tool'] = db_search_tool
...
def add_new_route_to_output_df(output_df: pd.DataFrame, row: Dict) -> pd.DataFrame:
    ...
    # order columns with smiles, inchi_key, error_type, error_message, retro_tool, db_search_tool always first
    try:
        output_df = output_df[
            ['smiles', 'inchi_key', 'error_type', 'error_message', 'retro_tool', 'db_search_tool'] + [col for col in output_df.columns if
                                                                      col not in ['smiles', 'inchi_key', 'error_type',
                                                                                  'error_message', 'retro_tool', 'db_search_tool']]]
    except KeyError:
        pass
    return output_df
...
def structure_route_outputs(error_message: str | None, error_type: str | None, output_df: pd.DataFrame,
    ...
    if check_route_to_add(workshop):  # True if route exists
        # Get tool information from workshop
        retro_tool = getattr(workshop, 'retro_tool', None)
        db_search_tool = getattr(workshop, 'db_search_tool', None)
        
        # Convert enum values to strings for output
        retro_tool_str = str(retro_tool) if retro_tool is not None else None
        db_search_tool_str = str(db_search_tool) if db_search_tool is not None else None
        ...
        reaction_info: Dict | None = add_route_info(reaction_names=workshop.reaction_names,
                                                    ...
                                                    retro_tool=retro_tool_str,
                                                    db_search_tool=db_search_tool_str)
