...
def check_template_paths(template_dir: str, csv_path: str) -> Set[str]:
    ...
    if 'template' not in df.columns:
        logger.error("The input csv does not contain a 'template' column.")
        raise ValueError("The input csv does not contain a 'template' column.")

    if not len(df[df['template'].notna()]):
        logger.error("The input csv does not contain template values.")
...
def check_apo_template(template_path: str) -> None:
    ...
    if not os.path.exists(template_path):
        logger.critical(f"The template {template_path} does not exist.")
...
def format_manual_route(row: pd.Series) -> Tuple[List[Tuple[Any, Any]], List[Any], int]:
    ...
    for step in range(1, last_step + 1):
        if step == 1:
            ...
            reactants.append(reactants_step)
        reaction_names.append(row[f'reaction_name_step{step}'].replace(' ', '_'))
