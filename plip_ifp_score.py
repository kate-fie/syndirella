...
class PlipIfpScore(Score_generic):
    ...
    def ifp_scorer(self, interactions_file):
        """
        Calculate the IFP score for the interactions of elaborated compounds compared to base compound (and fragments if provided?)
...
def main():
    parser = argparse.ArgumentParser('PLIP IFP score filter')
    parser.add_argument('--input_file', required=True, help='input sdf file containing molecules to filter')
    parser.add_argument('--output_file', required=True, help='output csv file to write quantified interactions')
    parser.add_argument('--results_dir', required=True, help='where to save the results')
